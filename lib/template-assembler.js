...
function getTemplatePaths(templatesFolder, templates, options, callback) {
    ...
    Async.each(templates, resolvePartials, err => {
        ...
        /***
         * Under docker, many paths on a mapped volume
         * may be reported missing even though they are
         * read in just fine.  This has to do with the level
         * of concurrency available on large systems.  In order
         * to prevent those false positives from slipping into
         * the error result, remove any "missing" templates that
         * were actually read in just fine.
         */
        let pathMap = templatePaths.reduce((result, value) => {
            result[value] = true;
            return result;
        }, {});
        let missing = missingTemplates.filter((x) => {
            return !(x in pathMap);
        });
        if (missing.length > 0) {
        ...
        Fs.readFile(file, {encoding: 'utf-8'}, (err, content) => {
            ...
            while (match !== null) {
                ...
                if (templatePaths.indexOf(partialPath) === -1) {
                    templateMatches[partialPath] = true;
                }
                match = partialRegex.exec(content);
            }
            Object.keys(partialPath).forEach((x) => {
                matches.push(x);
            });
